...
Coefficients of *Skewness* $g_1$, *Kurtosis* $g_2$ describe the symmetry and extremity of tails of the distribution.
...
geom_density(aes(fill=ind), alpha=0.8)+
labs(
  x=NULL,
  y=NULL
  )+
...
geom_boxplot(aes(fill=ind, outlier.color = ind), alpha=0.8)+
labs(
  x=NULL,
  y=NULL
  )+
...
A *Scatter plot* is a type of Plot using Cartesian coordinate system to display values for two variables for a set of data. It helps us:

-   take a short glance at effect of two variables.
-   estimate kinds of correlations between variables.
-   infer the direction of correlation (if any).
