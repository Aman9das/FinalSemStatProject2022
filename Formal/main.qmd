...
Also know that the Gross Domestic Product per capita of a country basically tells about the wealth of a country. So, in this project Our *Aim* is to determine how these features affect the *Health* of the citizens. out 
Life expectancy calculated for the year is based on the ages at the time of death of all the people who have deceased in that particular year. Also, in most cases it can tell about the current health condition of the country. Suppose the country is being hit by an epidemic, we can see that the people who might have lived longer if they had been unaffected by the epidemic. Hence we can use *Life Expectation* as a measurable proxy for health.
...
::: {.column width="50%"}
-   <p style="font-size:25pt;">ln(GDP) is nearly symmetrical, while Sanitation and Life Exp. are highly left-skewed.</p>
-   <p style="font-size:25pt;">This indicates majority of countries have good sanitation system, and citizen health.</p>
-   <p style="font-size:25pt;">ln(GDP) and Sanitation are platykurtic, while Life Exp. is leptokurtic.</p>
:::
:::
...
- Therefore, these plots confirm our previous inferences.
...
- The median seems to be in the centre of the quartiles for the ln(gdp) distribution, and it's closer to the 2nd quartile for the other two distributions.
- So, we can say that the distributions of the sanitation access and life expectancies are left skewed which further confirms the previous inferences.
- We observe one outlier in the Life Expectancy dataset. It is Haiti at 32.5 years. This is due to the cholera epidemic that broke out in the year 2010. Moreover this confirms our assumption that life expectancy is a very good proxy for health.

```{r}
d = subset(d, country!="Haiti")
```

# Scatter Plot

*Scatter plots* help us estimate the type of relationship between variables. They give a good intuition whether the variales are related or not.
...
```
- We observe from the scatter plot that the correlation between ln(gdp) and sanitation access appears to be mostly linear for countries with ln(gdp) less than 10. 
- Also, we see that the sanitation access is quite high for the countries with ln(gdp) above 10 for the countries with and is very close to 100% for countries with ln(gdp) above 11. 
...
```
- In this plot too, we can see that the correlation appears to be linear between life expectation and sanitation. We also observe that there is clustering around the top right side of the plot, which is supported by the box plots of both the distributions.
...
From the previous plots we get a strong intuition for a linear correlation between the variables.
This suggests us to perform linear regression over the plots in order to obtain the line of best fit.
