...
*Mean or Arithmetic Mean* $\bar{x}$,
*Geometric Mean* $\operatorname{GM}(x)$,
*Harmonic Mean* $\operatorname{HM}(x)$,
*Median* $\operatorname{median}(x)$ and
*Mode* $\operatorname{mode}(x)$ are some measures of *central tendency* in the sample.
...
Range range(x), Semi-int`..` SIR(x), Mean Deviation about x' $\operatorname{MD}_{(x')}(x)$, Variance $s_x^2$, Standard Deviation $s_x$ `...`
...
pcortab = data.frame(
  ...
  SPCor = c(
    spartcor[2, 1], # row number denotes the X, col number denotes the Y
    spartcor[3, 1],
    spartcor[3, 2]
  ),
  PCor = c(
