...
## Fitted lines on Scatter Plot

::: {.absolute top=87 left=0 width="600"}

```{r}
olsplot(d, d$lngdp, d$snt,
        "log of GDP per capita", "Sanitation access %")
```

:::

::: {.absolute top=405 left=316 width="600"}

```{r}
olsplot(d, d$lngdp, d$lfx,
        "log of GDP per capita", "Life Expectancy")
```

:::

::: {.absolute top=87 left=600 width="600"}

```{r}
olsplot(d, d$snt, d$lfx,
        "Sanitation access %", "Life Expectancy")
```
:::